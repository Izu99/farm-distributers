...
venderRoutes.route('/mysearchorders/:pathParam1?').get(function (req, res){
    ...
    // Orders.find({$and:[{date : search},{email : email}]},function (err,srch){
        Vender.find({$and:[{$or: [{vName: search}, {lName: search},{pNumber: search},{email: search}]}]},function (err,srch){ 
        if(err)
            console.log(err);
        else{
            res.json(srch)
