...
import FarmerSearchRow from "./FarmerThrowTable";

export default class FarmerSearch extends Component {
	constructor(props) {
		super(props);
		
        this.state = { farmerSearch: [] };
	...
	componentDidMount() {
        ...
        axios.get('http://localhost:4000/farmer/mysearchorders/'+this.props.match.params.pathParam1)
		
			.then((response) => {
			
				this.setState({ farmerSearch: response.data });
			})
	...
	tabRow() {
		return this.state.farmerSearch.map(function (object, i) {
			return <FarmerSearchRow obj={object} key={i} />;
		});
	...
	exportPDF = () => {
        ...
        const data = this.state.farmerSearch.map(elt=> [elt.vName, elt.lName,  elt.dob,elt.pNumber, elt.uName, elt.password, elt.email, elt.image]);
