...
import Vendorsearch from "./VendorThowTable";
...
export default class Vendor extends Component {
	...
	componentDidMount() {
        ...
        axios.get('http://localhost:4000/vender/mysearchorders/'+this.props.match.params.pathParam1)

			.then((response) => {
			
				this.setState({ vendorsearch: response.data });
			})
	...
	tabRow() {
		return this.state.vendorsearch.map(function (object, i) {
			return <Vendorsearch obj={object} key={i} />;
		});
	...
	exportPDF = () => {
        ...
        const data = this.state.vendorsearch.map(elt=> [elt.vName, elt.lName,  elt.dob,elt.pNumber, elt.uName, elt.password, elt.email, elt.image]);
